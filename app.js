...
const QUESTION_TEMPLATES = [
    ...
    },
    {
        ...
        inputs: [{ label: 'งาน \\( W \\) \\( (\\text{Joule}) \\):' }],
        text: (p) => `ระบบแก๊สหนึ่งคายความร้อนออกสู่สิ่งแวดล้อม \\(${p.r ? `(${p.q_base} + \\ ${p.r})` : p.q_mag}\\text{ J}\\) ในขณะเดียวกันพบว่าพลังงานภายในระบบลดลง \\(${p.dU_mag}\\text{ J}\\) งานที่เกี่ยวข้องมีค่ากี่จูล \\( (\\text{ระวังเครื่องหมายอุณหพลศาสตร์}) \) `,
        generate: (r) => {
    ...
    // อัตนัย 4: ปริมาตรคงที่ (Double)
    {
        ...
        text: (p) => `กระบอกสูบถูกยึดให้ **ปริมาตรคงที่** จากนั้นให้ความร้อนแก่ระบบแก๊สภายในจำนวน \\(${p.r ? `(${p.Q_base} + \\ ${p.r})` : p.Q}\\text{ J}\\) จงหางานที่แก๊สทำได้ \\( (W) \) และพลังงานภายในที่เปลี่ยนไป \\( (\\Delta U) \)`,
        generate: (r) => {
    ...
    // อัตนัย 6: อะเดียแบติก
    {
        ...
        inputs: [{ label: '\\( \\Delta U \\) \\( (\\text{Joule}) \\):' }],
        text: (p) => `กระบอกสูบหุ้มฉนวนความร้อนอย่างดี ถูกออกแรงบีบอัดอย่างรวดเร็วทำให้สิ่งแวดล้อมทำงานให้แก๊ส \\(${p.r ? `(${p.W_base} + \\ ${p.r})` : p.W}\\text{ J}\\) พลังงานภายในระบบแก๊สเปลี่ยนไปเท่าใด \\text{สังเกตเครื่องหมาย}`,
        generate: (r) => {
